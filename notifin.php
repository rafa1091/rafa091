...
// Periksa apakah user sudah login sebagai trainer
if (!isset($_SESSION['trainer_username'])) {
    header("Location: login.php");
    exit();
}

// Proses input notifikasi jika form disubmit
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['judul']) && !empty($_POST['pesan'])) {
        $judul = mysqli_real_escape_string($koneksi, $_POST['judul']);
        $pesan = mysqli_real_escape_string($koneksi, $_POST['pesan']);
        
        $query = "INSERT INTO notifikasi (judul, pesan, tanggal) VALUES ('$judul', '$pesan', NOW())";
